#ifndef _HPP_MAIN_MULTIBOOT_
#define _HPP_MAIN_MULTIBOOT_
...
namespace multiboot {
     ...
     *
     * Entries in this array are always `sizeof(@ref mm_entry_t)` bytes long, no matter what the structure itself says.
     *  The size field will not be updated to respect this fact. The first entry in these objects (that is, offset 0)
     *  will be the `size` value.
     */
    extern entry_t mem_table[LOCAL_MM_COUNT];
    /** The command line as given by the multiboot bootloader.
     *
     * It will be null terminated, even if the original is longer than @ref LOCAL_BOOT_LOADER_NAME_LENGTH or the
     *  original string is not terminated (in which case you likely have lots of garbage data).
     */
    extern char cmdline[LOCAL_CMDLINE_LENGTH];
