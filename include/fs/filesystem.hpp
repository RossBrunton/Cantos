...
/** Provides the base classes for managing filesystems and their underlying storage */
...
/** Inode type enum
 *
 * Each Inode must be of one of these types
 */
enum class InodeType { DIRECTORY, FILE };

/** An entry in a directory's Inode list */
struct InodeEntry {
    uint64_t inode; /**< The inode number of the child */
    Utf8 name; /**< The filename of the child */
};

/** A single inode
 *
 * Filesystems are made up of inodes in a tree structure, with DIRECTORY inodes being the only ones with children.
 *
 * Each inode can be uniquely identified using it's inode number and its filesystem.
 *
 * Each inode has contents (i.e. File contents, which are stored as an @ref object::Object.
 */
class Inode {
public:
...
public:
    ...
    virtual shared_ptr<Storage> get_storage() { return storage; }
...
public:
    ...
    FilePathEntry(Utf8 name = Utf8(""), shared_ptr<FilePathEntry> parent = nullptr, shared_ptr<Inode> inode = nullptr);
